...
SESSION_PREFIX="family-spark-dev"
MAIN_DEFAULT_PORT=8091
WORKTREE_PORT_MIN=8092
WORKTREE_PORT_MAX=8097
...
usage() {
  cat <<EOF >&2
Usage:
  ${SCRIPT_NAME} <start|stop|restart|logs|status> [port]

Examples:
  ${SCRIPT_NAME} start
  ${SCRIPT_NAME} restart 8091
  ${SCRIPT_NAME} logs

Notes:
  - Main checkout defaults to port ${MAIN_DEFAULT_PORT}.
  - Codex worktree checkouts auto-select a free port in ${WORKTREE_PORT_MIN}-${WORKTREE_PORT_MAX}.
...
start_session() {
  ...
  command="$(printf 'cd %q && npm run dev -- --host localhost --port %q 2>&1 | tee %q' "${ROOT_DIR}" "${port}" "${log_file}")"
...
status_sessions() {
  ...
  if (( any == 0 )); then
    echo "No managed ${SESSION_PREFIX} tmux sessions are running."
  fi
}
